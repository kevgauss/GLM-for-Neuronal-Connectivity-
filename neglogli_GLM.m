...
% fprintf('Stim filtered with k output: I0 =%.3f\n', I0);
% fprintf('Sum of all filter outputs: Itot =%.3f\n', Itot);
...
% fprintf('Conditional intensity: rr =%.3f\n', rr);
...
% fprintf('rate in mask: rr(bmask) =%.3f\n', rr(bmask));
trm1 = sum(rrmask)*dt;  % non-spiking term
fprintf('Non spiking term: trm1 =%.3f\n', trm1);
trm2 = -sum(log(rrmask(gg.sps(bmask)>0)));  % Spiking term
fprintf('Spiking term: trm2 =%.3f\n', trm2);
neglogli = trm1 + trm2;
